...
CREATE Procedure[HSTG].[LOAD_HSTG_CA_SOMWEB]
AS 
BEGIN
    ...
    EXEC "HSTG"."LOAD_HSTG_CA_SOMWEB";
    
    Source                   Destination             
    ---------------------    ---------------------- 
    STG"."STG_CA_SOMWEB"    "HSTG"."HSTG_CA_SOMWEB" 
    ...
    */
    ...
    BEGIN TRY
        ...
        SELECT @ROW_COUNT = COUNT_BIG(*)  FROM  "STG"."STG_CA_SOMWEB"
        IF (@ROW_COUNT > 0)
        BEGIN
        /* Step 2:  IMPORT FROM STAGING TO HISTORY */
                INSERT INTO "HSTG"."HSTG_CA_SOMWEB"
                (
                    "So#"
                    ,"Cust#"
                    ,"Order Date"
                ...
                FROM "STG"."STG_CA_SOMWEB" S
                WHERE NOT EXISTS(SELECT 1 FROM "HSTG"."HSTG_CA_SOMWEB" H WHERE H.OMD_CURRENT_RECORD_INDICATOR = 'Y' AND OMD_IS_RECORD_DELETED = 'N' AND H.OMD_HASH_FULL_RECORD = S.OMD_HASH_FULL_RECORD)
            ...
            /* Step 3:  IMPORT DELETED RECORDS FROM HISTORY TO HISTORY */
                ...
                SELECT 
                    ...
                    ,'Y' AS "OMD_CURRENT_RECORD_INDICATOR"
                FROM "HSTG"."HSTG_CA_SOMWEB" H
                Where "OMD_INSERT_DATETIME" >= Dateadd(Month, -6, getdate())
					And NOT EXISTS (SELECT 1 FROM "STG"."STG_CA_SOMWEB" S WHERE 
            ...
            /* Step 4:  UPDATE OMD_CURRENT_RECORD_INDICATOR FLAG*/
                ...
                WHERE "OMD_IS_RECORD_DELETED" = 'N'
					AND EXISTS (SELECT 1 FROM "STG"."STG_CA_SOMWEB" S WHERE 
                    COALESCE(H."SO#",'''') = COALESCE(S."SO#",'''')
            )
					AND NOT EXISTS (SELECT 1 FROM "STG"."STG_CA_SOMWEB" S WHERE H.OMD_HASH_FULL_RECORD = S.OMD_HASH_FULL_RECORD)
            
                UPDATE  H SET OMD_CURRENT_RECORD_INDICATOR='N' 
                FROM "HSTG"."HSTG_CA_SOMWEB" H
                WHERE "OMD_IS_RECORD_DELETED" = 'Y' AND EXISTS (SELECT 1 FROM "STG"."STG_CA_SOMWEB" S WHERE 
                    COALESCE(H."SO#",'''') = COALESCE(S."SO#",'''')
            )

			Update HSTG.HSTG_CA_SOMWEB 
			Set OMD_CURRENT_RECORD_INDICATOR = 'N'
			Where OMD_CURRENT_RECORD_INDICATOR = 'Y' And OMD_IS_RECORD_DELETED = 'N' 
				And Exists (Select *
							From HSTG.HSTG_CA_SOMWEB T (nolock)
							Where OMD_CURRENT_RECORD_INDICATOR = 'Y' And OMD_IS_RECORD_DELETED = 'Y' 
									And T.[So#] = HSTG_CA_SOMWEB.[So#])
