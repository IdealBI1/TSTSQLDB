...
CREATE Procedure[HSTG].[LOAD_HSTG_CA_OELREB]
AS 
BEGIN
    ...
    EXEC "HSTG"."LOAD_HSTG_CA_OELREB";
    
    Source                   Destination             
    ---------------------    ---------------------- 
    STG"."STG_CA_OELREB"    "HSTG"."HSTG_CA_OELREB" 
    ...
    */
    ...
    BEGIN TRY
        ...
        SELECT @ROW_COUNT = COUNT_BIG(*)  FROM  "STG"."STG_CA_OELREB"
        IF (@ROW_COUNT > 0)
        BEGIN
        /* Step 2:  IMPORT FROM STAGING TO HISTORY */
                INSERT INTO "HSTG"."HSTG_CA_OELREB"
                (
                    "Invoice#"
                    ,"Inl#"
                    ,"T"
                ...
                FROM "STG"."STG_CA_OELREB" S
                WHERE NOT EXISTS(SELECT 1 FROM "HSTG"."HSTG_CA_OELREB" H WHERE H.OMD_CURRENT_RECORD_INDICATOR = 'Y' AND OMD_IS_RECORD_DELETED = 'N' AND H.OMD_HASH_FULL_RECORD = S.OMD_HASH_FULL_RECORD)
            ...
            /* Step 3:  IMPORT DELETED RECORDS FROM HISTORY TO HISTORY */
                ...
                SELECT 
                    ...
                    ,'Y' AS "OMD_CURRENT_RECORD_INDICATOR"
                FROM "HSTG"."HSTG_CA_OELREB" H
                Where "OMD_INSERT_DATETIME" >= Dateadd(Month, -6, getdate())
					And NOT EXISTS (SELECT 1 FROM "STG"."STG_CA_OELREB" S WHERE 
            ...
            /* Step 4:  UPDATE OMD_CURRENT_RECORD_INDICATOR FLAG*/
                ...
                WHERE "OMD_IS_RECORD_DELETED" = 'N'
					AND EXISTS (SELECT 1 FROM "STG"."STG_CA_OELREB" S WHERE 
                    COALESCE(H."INVOICE#",0) = COALESCE(S."INVOICE#",0)
                    AND COALESCE(H."INL#",0) = COALESCE(S."INL#",0)
                    AND COALESCE(H."T",'''') = COALESCE(S."T",'''')
            )
					AND NOT EXISTS (SELECT 1 FROM "STG"."STG_CA_OELREB" S WHERE H.OMD_HASH_FULL_RECORD = S.OMD_HASH_FULL_RECORD)
            
                UPDATE  H SET OMD_CURRENT_RECORD_INDICATOR='N' 
                FROM "HSTG"."HSTG_CA_OELREB" H
                WHERE "OMD_IS_RECORD_DELETED" = 'Y' AND EXISTS (SELECT 1 FROM "STG"."STG_CA_OELREB" S WHERE 
                    COALESCE(H."INVOICE#",0) = COALESCE(S."INVOICE#",0)
                    AND COALESCE(H."INL#",0) = COALESCE(S."INL#",0)
                    AND COALESCE(H."T",'''') = COALESCE(S."T",'''')
            )

			Update HSTG.HSTG_CA_OELREB 
			Set OMD_CURRENT_RECORD_INDICATOR = 'N'
			Where OMD_CURRENT_RECORD_INDICATOR = 'Y' And OMD_IS_RECORD_DELETED = 'N' 
				And Exists (Select *
							From HSTG.HSTG_CA_OELREB T (nolock)
							Where OMD_CURRENT_RECORD_INDICATOR = 'Y' And OMD_IS_RECORD_DELETED = 'Y' 
									And T.[Invoice#] = HSTG_CA_OELREB.[Invoice#] 
									And T.[Inl#] = HSTG_CA_OELREB.[Inl#] 
									And T.[T] = HSTG_CA_OELREB.[T])
